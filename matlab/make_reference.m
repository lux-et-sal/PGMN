...
acc = struct();
% The production arrays carry RMSE and Bias per cell; ubRMSE, the metric
% the paper reports, follows exactly from the two: ubRMSE^2 = RMSE^2 - Bias^2.
acc.wbm_baseline        = summarize4(P.bias_wbm_per_grid, P.rmse_wbm_per_grid, ...
                                     P.r_wbm_per_grid,    P.kge_wbm_per_grid);
acc.pgmn_reconstruction = summarize4(P.bias_mdn_per_grid, P.rmse_mdn_per_grid, ...
                                     P.r_mdn_per_grid,    P.kge_mdn_per_grid);
...
ref.tolerance = struct('Bias', 1e-4, 'ubRMSE', 1e-4, 'R', 1e-3, 'KGE', 1e-3, ...
...
function s = summarize4(bias, rmse, r, kge)
    s = struct();
    ubrmse = sqrt(max(double(rmse(:)).^2 - double(bias(:)).^2, 0));
