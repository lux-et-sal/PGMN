...
%   precip        [H,W,T] double   MSWEP precipitation (mm/day), NaN allowed
%                 ** TIME CONVENTION -- read this before supplying precip **
%                 precip(:,:,t) is the rainfall that falls BETWEEN SMAP
%                 observation t and observation t+1. It is NOT 'the rainfall
%                 of calendar day t'. The forward recursion in
%                 wbm_forwardSim consumes pp(j-1) to advance sms(j-1) to
%                 sms(j), so index t-1 is what drives the state at t.
%
%                 Which raw MSWEP day belongs at index t depends on the
%                 cell, because SMAP's descending overpass lands anywhere
%                 between 01 and 23 UTC while MSWEP accumulates 00-24 UTC:
%                   overpass hour < 12  ->  raw MSWEP day t
%                   overpass hour >= 12 ->  raw MSWEP day t+1
%                 The production pipeline resolves this per cell in
%                 Satellite_Data_Preprocess_Code_36km_EASE/
%                 o5_Align_MSWEP_to_SMAP_Scantime_M36.m and ships the result
%                 as MSWEP_ScantimeAligned_M36.mat. Supplying raw, unaligned
%                 MSWEP here reproduces the pre-2026-09 behaviour, in which
%                 the model was forced one to two days late.
...
%     the MATLAB path. wbm_forwardSim.m in turn depends on wbm_quantreg.m.
    ...
    % Sliced as double, not single. Every downstream solver here uses finite
    % differences or a simplex; at single precision (~1.2e-7 relative) the
    % objective difference over a 1.5e-8 step is pure rounding noise, and the
    % search terminates at its starting point. In the production calibration
    % this silently pinned 69% of cells at the initial guess
    % (alpha = 2.000, Z = 330.34 mm). See WBM/main_wbm_open_loop.m.
    sm_cells = zeros(T, n_cells, 'double');
    pp_cells = zeros(T, n_cells, 'double');
    for k = 1:n_cells
        sm_cells(:, k) = double(squeeze(smap  (rows(k), cols(k), :)));
        pp_cells(:, k) = double(squeeze(precip(rows(k), cols(k), :)));
    end
    phi_cells   = double(porosity(sub2ind([H, W], rows, cols)));
    alpha_cells = double(alpha   (sub2ind([H, W], rows, cols)));
    Z_cells     = double(Z       (sub2ind([H, W], rows, cols)));
    beta_cells  = double(beta    (sub2ind([H, W], rows, cols)));
