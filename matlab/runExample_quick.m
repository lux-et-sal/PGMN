...
fprintf('  grid          : %s\n', S.meta.grid);
fprintf('  mixture       : M = %d, %d fitted weights\n', M, model.performance.num_params);
...
dr = metrics_mdn.per_grid.RMSE - metrics_wbm.per_grid.RMSE;
pr = 100 * (1 - metrics_mdn.per_grid.RMSE ./ metrics_wbm.per_grid.RMSE);
fprintf('  delta KGE       = %+.4f\n', median(dk(isfinite(dk))));
fprintf('  delta RMSE      = %+.4f  (m^3 m^-3)\n', median(dr(isfinite(dr))));
fprintf('  RMSE reduction  = %+.1f %%\n\n', median(pr(isfinite(pr))));
