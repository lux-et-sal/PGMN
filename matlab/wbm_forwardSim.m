...
%               ** the precip column is the rainfall between row t and row
%               t+1, not the rainfall of calendar day t. The forward loop
%               below uses pp(j-1) to advance sms(j-1) -> sms(j), and the
%               backward loop inverts the same step with pp(j). Shifting the
%               column by one day silently biases every simulated series.
%               See wbm_simulate.m for how the per-cell alignment is built. **
